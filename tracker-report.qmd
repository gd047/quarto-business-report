...
```{r}
#| label: helpers
format_datetime <- stamp("Κυριακή, 1 Δεκ., 2000 11:34 μμ")
run_time <- now()
```
...
Δοκιμή εμφάνισης Ελληνικών. Please find below the latest `r reportLabFormatted` for `r lab` as of `r format_datetime(run_time)`.
